...
CreateOperation::CreateOperation(Graphic::GlyphPtr glyphParent, MainWindow *parent)
	: QDialog(parent), parent_(glyphParent)
{
	text_ = new QLineEdit();
	hint_ = new QLabel(tr("Operation: "));
...
void CreateOperation::Create()
{
	QString text(text_->text());

	if(!text.isEmpty())
	{
		emit CreateOver(new Graphic::Operation(parent_, text));
		close();
	}
}
